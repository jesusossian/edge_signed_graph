...
export edgeForm1, edgeForm2, edgeForm3, stdVars

function edgeForm1(inst::instData, params::parameterData)
    ...
    @variable(model, 0.0 <= y[edges(inst.G)] <= 1.0)
    ...
    # write_to_file(model,"edge1_model.lp")
...
function edgeForm2(inst::instData, params::parameterData)
    ...
    @variable(model, x[vk], Bin)
    #@variable(model, y[edges(inst.G)], Bin)
    @variable(model, 0 <= y[edges(inst.G)] <= 1.0)
    ...
    for e in EP
        ...
        for i in 1:k
            for j in 1:k
                if i != j
                    @constraint(model, x[(u,i)] + x[(v,j)] <= 1 + y[e])
                end
            end
        end
    end
      
    # write_to_file(model,"edge2_model.lp")
...
function edgeForm3(inst::instData, params::parameterData)
    ...
    # write_to_file(model,"edge3_model.lp")
