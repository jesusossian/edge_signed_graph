...
if __name__ == "__main__":
    ...
    for e in EP:
        for i in K:
            for j in K: 
                if i != j:
                    model.addConstr(x[(e[0],i)] + x[(e[1],j)] <= 1 + y[(e[0],e[1])], "con6")

    if method != "mip":
        for v in model.getVars():
            v.setAttr('vtype', 'C')

    #model.write("edge3_signed.lp")
    ...
    # export solution
    if method == "mip":
        arq = open(os.path.join(results_path,f'{method}_n{n}_edge3_signed_.txt'),'a')
        arq.write(instance+';'
        +str(round(objval,2))+';'
        ...
        arq = open(os.path.join(results_path,f'{method}_n{n}_edge3_signed.txt'),'a')
        arq.write(instance+';'
        +str(round(objval,2))+';'
        +str(round(runtime,2))+'\n')
